...
Eventually I want driver matching (Hungarian algorithm, very exciting, another post) but you can't match drivers to riders if you don't know where anybody *is*. So step one: let a user report their lat/lng.
...
Took embarrassingly long before I actually looked at how `GetUserByID` gets its data:
...
This one *validates itself*. The response echoes back the client's own input. You POST `1.35, 103.8`, you get back `1.35, 103.8`, your brain goes "yep, works", and you move on.
...
Think of it like writing a note on a photocopy of a page and then filing the original. The photocopy says exactly what you wanted it to say! You can hold it up and show people! It's just not the thing anyone will ever read again.

Or in test form:

- test that POSTs a location and checks the response → **passes**
- test that POSTs a location and then GETs it → **fails**

guess which one you write first..
...
## ok but... that's duplicated lookup, no?
...
I still think it's a bad idea. Two reasons — one about design, one specifically about Go.

## reason 1: who is allowed to change a user?

This is encapsulation, but not the boring "make your fields lowercase" version. The question is: **where does the authority to mutate a user live?**

With value-returning getters and dedicated writers, that question has a finite answer you can grep for. It's whatever `Update*` methods exist on `UserRepository`. Read the method list, you know the whole story. Every mutation is named and each one does its own lookup and its own write in one auditable place.
...
A *read* operation has quietly become a write capability handout. And nothing in the signature warns you — `*models.User` reads like a performance optimisation, not like "here, you may now edit the database".
...
## reason 2: `append` moves the furniture
...
Note what did *not* happen. No crash. No nil deref. Go's GC keeps the old array alive **precisely because** your pointer still references it. So the pointer stays perfectly readable and writable — it's just pointing at an orphaned copy that nobody will ever read again.
